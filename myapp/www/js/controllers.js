...
.controller('CourseLectureCtrl', function($scope, $stateParams, student) {
  ...
  student.getLectures($stateParams.course_id, function(lectures) {
...
.controller('YoutubeTestCtrl', function($scope, $http, student) {
    ...
    $scope.youtubeParams = {
      key: 'AIzaSyAWSkVamF5NZpdbR9xPVAAP8kOeQCXLTmA',
      type: 'video',
      maxResults: '5',
      part: 'id,snippet',
      q: 'creatorup',
...
.controller('LectureCtrl', ['$scope', 'student', '$sce', '$rootScope', '$state',
  '$stateParams', 'MockService', '$ionicActionSheet',
  '$ionicPopup', '$ionicScrollDelegate', '$timeout', '$interval',
  function($scope, student, $sce, $rootScope, $state, $stateParams, MockService,
    $ionicActionSheet,
    $ionicPopup, $ionicScrollDelegate, $timeout, $interval) {

    $scope.lecture_title = $stateParams.lecture_title;
    $scope.lecture_id = $stateParams.lecture_id;
    
    student.getLecture($scope.lecture_id, function(lecture) {
      
      $scope.lecture_url = lecture.youtubeLink;
      console.log($scope.lecture_url)
        
      $scope.youtubeLink2 = lecture.youtubeLink;    
      var newLink = lecture.youtubeLink.replace("watch?v=", "v/");
      lecture.youtubeLink = newLink;
      $scope.lecture = lecture;

      $scope.lecture.youtubeLink = $sce.trustAsResourceUrl(lecture.youtubeLink);
        
    })

    student.getNotes($scope.lecture_id, function(notes) {
      $scope.notes = notes[0].notes;
    });
    ...
    $scope.saveNotes = function() {
      ...
      /*
        student.saveNotes($scope.notes, $scope.lecture.id, function() {
            console.log("Successfully saved notes.");
          })
      */

      student.saveNotes($scope.notes, $scope.lecture.id, $scope.lecture.lecture_no, $scope.lecture.title, $scope.youtubeLink2, $scope.lecture.course, $scope.lecture.keywords, function() {
        console.log("Successfully saved notes.")
      })
  ...
  function($http, $q) {
    ...
    me.getMockMessage = function() {
      return {
        ...
        date: new Date(),
        text: 'Yes, excellent question! Let me link you to a page that can tell you more about neural networks.'
      };
    }
